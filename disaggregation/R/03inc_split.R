...
tb <- load_gtb('tb',convert_dashes=TRUE)     # includes notifications
pop <- load_gtb('pop',convert_dashes=TRUE)   # population for denom
if(noests.yet){
...
#' Make a directory for this output if it isn't there
drnm <- here('disaggregation/output/incsplits/')
...
#' Record out:
(knotem.now <- tb[year==estyr & !is.na(newrel.m014) & is.na(newrel.m04),
                unique(iso3)])
(knotem.then <- tb[year==estyr-1 & !is.na(newrel.m014) & is.na(newrel.m04),
                unique(iso3)])

drnm <- here('disaggregation/output/incsplits/data/')
if(!file.exists(drnm)) dir.create(drnm)
cat(knotem.now,file=here('disaggregation/output/incsplits/data/knotem.now.txt'))
cat(knotem.then,file=here('disaggregation/output/incsplits/data/knotem.then.txt'))
...
#' Record out:
(knotef.now <- tb[year==estyr & !is.na(newrel.f014) & is.na(newrel.f04),unique(iso3)])
(knotef.then <- tb[year==estyr-1 & !is.na(newrel.f014) & is.na(newrel.f04),unique(iso3)])

cat(knotef.now,file=here('disaggregation/output/incsplits/data/knotef.now.txt'))
cat(knotef.then,file=here('disaggregation/output/incsplits/data/knotef.then.txt'))
...
bn <- c(fn,mn)
(v <- tb[iso3=='UGA' & year==2022][,..bn])
sum(v,na.rm=TRUE)
tb[iso3=='UGA' & year==2022,c.newinc]

#' Split UGA by AFR ratio in these groups calculated via meta-analysis
#'
#' 
tmp <- tb[year==2022 & g.whoregion=='AFR'][,.(newrel.m5564,newrel.m65,newrel.f5564,newrel.f65)]

## F
tmpf <- tmp[!is.na(newrel.f5564+newrel.f65)][newrel.f5564+newrel.f65>0]
tmpf[,c('xi','ni'):=.(newrel.f5564,newrel.f5564+newrel.f65)]
resf <- rma.glmm(measure="PLO", xi=xi, ni=ni, data=tmpf)
resf <- predict(resf, transf=transf.ilogit)$pred

## M
tmpm <- tmp[!is.na(newrel.m5564+newrel.m65)][newrel.m5564+newrel.m65>0]
tmpm[,c('xi','ni'):=.(newrel.m5564,newrel.m5564+newrel.m65)]
resm <- rma.glmm(measure="PLO", xi=xi, ni=ni, data=tmpm)
resm <- predict(resm, transf=transf.ilogit)$pred

#' correct
tb[iso3=='UGA' & year==2022,.(newrel.m5564,newrel.m65,newrel.f5564,newrel.f65)]
tb[iso3=='UGA' & year==2022,c('newrel.f5564','newrel.f65'):=.(newrel.f5564*resf,newrel.f5564*(1-resf))]
tb[iso3=='UGA' & year==2022,c('newrel.m5564','newrel.m65'):=.(newrel.m5564*resm,newrel.m5564*(1-resm))]
tb[iso3=='UGA' & year==2022,.(newrel.m5564,newrel.m65,newrel.f5564,newrel.f65)]
...
#'
#' CDRs of countries
cdrtab <- est[,.(iso3,year,inc.num=inc*pop)] #NOTE inc.num out by factro
...
est[year==estyr & source.inc=="Country model",iso3] #COVID disruption, directly
est[year==estyr & source.inc=="Regional model",iso3] #COVID disruption, more crudely
est[year==estyr & source.inc=="Current trends",iso3] #
est[year==estyr & grepl("adjustment",source.inc),iso3] #
est[year==estyr & grepl("opinion",source.inc),iso3] #

#' standard factor
isosf <- est[grepl("adjustment",source.inc),
             as.character(unique(iso3))]
cdrtab[isosf]

#' inventory study
isois <- est[grepl("Inventory",source.inc),
             as.character(unique(iso3))]
cdrtab[isois]
...
isopass1 <- c(cdrtab[isosf][cdr>0.85,iso3], #SF + CDR>0.85
              cdrtab[isois][cdr>0.85,iso3], #IS + CDR>0.85
...
incsplit[!is.finite(inc)] #nothing :-)
...
#' test!
tmp <- priorsample('NGA',silent=FALSE)
niplot(tmp)
...
#'
#' Avoiding problem with SCG
isotodo <- isotodo[!isotodo %in% c('SCG')] #premerge: SCG now separate!
for(cn in isotodo){
  cno <- priorsample(cn)
  cno$pass <- 2
  ISL[[cn]] <- copy(cno)
...
#' Save out plot
#' 
fn <- here('disaggregation/output/incsplits/aplots')
...
fn <- here('disaggregation/reportoutput')
if(!file.exists(fn)) dir.create(fn)

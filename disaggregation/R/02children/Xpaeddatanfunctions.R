...
noests.yet <- FALSE
if(noests.yet){
  load(here('drtb/tmpdata/unaids.rda'))
...
if(!file.exists(here('disaggregation/R/02children/data/unaids.Rdata'))){
  fn <- here('data/unaids/HIV2023Estimates_ForTBmodel_AllCountries_Embargoed13July_23June2023.xlsx')
  ## reformat:
  ...
  save(U,file=here('disaggregation/R/02children/data/U.Rdata'))
  ## first page
  ...
  Y <- Y[country!='Global']
  Y <- Y[,.(year,iso3,
            pop014=as.numeric(`N- Population aged 0-14 Male+Female`),
            hiv014=as.numeric(`N- HIV population (0-14) Male+Female`),
            hiv014.lo=as.numeric(`N- HIV population (0-14) Male+Female; Lower bound`),
            hiv014.hi=as.numeric(`N- HIV population (0-14) Male+Female; Upper bound`))]
  Y[,c('hiv014','hiv014.lo','hiv014.hi'):=.(hiv014/pop014,hiv014.lo/pop014,hiv014.hi/pop014)]
  unaids <- Y
  save(unaids,file=here('disaggregation/R/02children/data/unaids.Rdata'))
} else {
...
if(!file.exists(BCGfn)){
  ...
  BCG <- as.data.table(bcg_gho_country)
  BCG <- BCG[year==max(year),.(iso3,bcg=bcg_coverage)]
  save(BCG,file=BCGfn)
} else{
  load(BCGfn)
}
...
estold <- estold[year==max(year)][,.(iso3,oldinc=inc)]

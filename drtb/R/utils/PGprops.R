...
dre <- dre[!is.na(g.mdr)] #drops ANT, SCG
reg <- unique(dre$g.mdr)
...
A <- dre$prop.rr.ret[sel]
B <- dre$prop.rr.ret.sd[sel]
A[A<1e-6] <- B[A<1e-6]/2                                    #approx for zeros
out <- vlohi(A,B)
dre[sel, prop.rr.ret.lo := out[1,]]
dre[sel, prop.rr.ret.hi := out[2,]]
dre[prop.rr.ret<1e-6,prop.rr.ret:=(prop.rr.ret.hi+prop.rr.ret.lo)/2]

...
subnationalkeepers <- c(subnationalkeepers,'PAK') #TODO check
...
ymnvar <- YB[iso3=='PHL',.(Vy=mean(((mdr_pcnt_lo-mdr_pcnt_hi)/392)^2)),by=patients]
lmnvar <- LB[iso3=='PHL',.(Vl=mean((rr*(r_rlt-rr))/r_rlt^3)),by=patients]
...
## get the scaling to equivalize the mean variance for Y vs L
ymnvar <- YB[iso3=='ZMB',.(Vy=mean(((rr_pcnt_lo-rr_pcnt_hi)/392)^2,na.rm=TRUE)),by=patients]

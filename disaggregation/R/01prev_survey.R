...
pop <- load_gtb('pop')       #NOTE have changed to use underscore names by hand
tb <- load_gtb('tb')
...
ggplot(data=prev[sex=='a' &
                 age_group %ni% xclude],
       aes(x=age_group,y=prev,col=case_type)) +
  geom_errorbar(aes(ymin=prev_lo,ymax=prev_hi),width=0) + 
  geom_point() +
...
#'
prev[age_group %ni% xclude,{print(paste0(iso3,'=',length(unique(age_group))))},by=iso3]
prev[age_group %ni% xclude,{print(paste0(iso3,'=',length(unique(age_group))));
        print(sort(unique(age_group)));},by=iso3]
...
## GMB
tmp <- prev[iso3=='GMB' & case_type=='b' & area_type=='a' & sex=='a']
tmp <- tmp[age_group!='15plus',]
tmp2 <- copy(tmp)
tmp$age_group <- c('15_24','35_44','55_64')
tmp2$age_group <- c('25_34','45_54','65plus')
prev <- prev[!(iso3=='GMB' & sex=='a'),]    #drop these records
prev <- rbind(prev,tmp,tmp2)                  #join back on modified

## RWA
tmp <- prev[iso3=='RWA' & case_type=='b' & sex=='a']
tmp <- tmp[age_group!='15plus',]
tmp2 <- copy(tmp)
tmp$age_group <- c('15_24','35_44','55_64')
tmp2$age_group <- c('25_34','45_54','65plus')
...
prev[age_group %ni% xclude,
{if(length(unique(age_group)) !=6){print(paste0(iso3,'=',length(unique(age_group))))};},by=iso3]
...
prev <- prev[!(iso3=='TZA' & case_type=='b')] #drop the overall B+
prev <- prev[!(iso3=='TZA' & age_group=='15plus' & sex=='a')] #drop the overall smr+
prev[iso3=='TZA' & sex=='a',case_type:='b']

#' Check new surveys
#' "IND" "LSO" "MMR" "MOZ" "NAM" "NPL" "SWZ" "VNM" "ZAF"
prev[iso3=='IND']                       #OK
...
PR <- prev[sex=='a' &
        age_group %ni% xclude &
        case_type=='b']
PR$case_type <- NULL                    #since no longer true
PR[,unique(area_type)]                  #OK
...
PR[,RR:=.SD[,prev]/.SD[age_group=='15_24',prev],by=iso3.year] #calculate RR
PR[,prevse:=(prev_hi-prev_lo)/3.92]                           #prevalence SE

#' Want to include errors from reference category and others for $Q=X/Y$ as $var(Q)/\bar{Q}^2=(var(X)/\bar{X}^2+\var(Y)/\bar{Y}^2)^2$
PR[,RRse:=.SD[age_group=='15_24',prevse/prev],by=iso3.year] #reference group errors
PR[age_group=='15_24',RRse:=0]          #no error in reference group
...
ggplot(data=PR,
       aes(x=age_group,y=RR,col=iso3.year,group=iso3.year,ymin=RRlo,ymax=RRhi)) +
...
regkey <- unique(pop[,.(iso3,g_whoregion)])
PR <- merge(PR,regkey,by='iso3',all.x=TRUE)
PR$age_group <- factor(PR$age_group)    #this wasn't a factor before


#' In terms of simple predictors of this pattern, there is some degree of cohesiveness by region:`g_whoregion`:
ggplot(data=PR,
       aes(x=age_group,y=RR,col=iso3.year,group=iso3.year)) +
...
getZdata <- function(reg){
    Z.df <- dcast(data=PR[g_whoregion==reg,],formula=iso3.year ~ age_group,value.var='RR')
    Z.se.df <- dcast(data=PR[g_whoregion==reg,],formula=iso3.year ~ age_group,value.var='RRse')
...
makeplotdata <- function(LoV,isoy=NA,reg='AFR',n=NULL){
    ...
    names(dfm)[2:3] <- c('age_group','logRR')
    dfm$age_group <- factor(dfm$age_group,
...
makeregionplot <- function(xtradata,reg='AFR',alph=.1){
 PP <-  ggplot(data=PR[g_whoregion==reg],
               aes(x=age_group,y=logRR,col=iso3.year,fill=iso3.year,
                   group=g_whoregion)) + 
     facet_wrap(~g_whoregion)+
...
ggplot(data=prev[sex%in%c('f','m'),],
       aes(x=sex,y=prev,col=case_type,pch=age_group)) +
    geom_errorbar(aes(ymin=prev_lo,ymax=prev_hi),width=0) +
    geom_point() +
...
MFD[,table(case_type,iso3)]

MFD[iso3=='TZA',case_type:='b'] #keep this as only available
MFD <- MFD[case_type!='s',]                        #ditch rest
## drop korea as different age group and no precision
MFD <- MFD[age_group=='15plus',]
MFD[,MF:=prev/.SD[sex=='f',prev],by=iso3.year]
MFD[,MFse:=(prev_hi-prev_lo)/(3.92*prev)] #fractional SEs
...
KH <- data.table(g_whoregion=c('SEA','WPR','AFR','EMR','AMR','GLO'),
...
SampleRRs <- function(n=1,iso3,
                      verbose=TRUE,
                      noest=FALSE #do as if no est present
                      ){
  if(verbose) cat('Country=',iso3,'\n')
  reg <- as.character(pop[iso3,g_whoregion][1]) #look up region
  ...
  print(srcinc)
  if(is.na(srcinc) |
     !(grepl('Standard adjustment',srcinc) | grepl('Inventory',srcinc)) & reg %ni% c('EUR','AMR') |
     srcinc == '*** Test! ***'){
    ...
    yr <- estyr
    if(is.na(sum(tb[iso3==iso3&year==yr,paste0('newrel_m',agz),with=FALSE]))){
      yr <- (estyr-1)
      cat('Check notification data! Using last year due to NAs!\n')
    }
    loc <- tb[iso3]
    tmp <- unlist(loc[year==yr,paste0('newrel_m',agz),with=FALSE])
    notes[1,] <- tmp
    tmp <- unlist(loc[year==yr,paste0('newrel_f',agz),with=FALSE])
    notes[2,] <- tmp
...
## checks
test <- SampleRRs(1,'PAK',noest=noests.yet)              #EMR, PS
test <- SampleRRs(1,'AFG',noest=noests.yet)              #EMR, no PS
test <- SampleRRs(1,'ZWE',noest=noests.yet)              #AFR, PS
test <- SampleRRs(1,'ZAF',noest=noests.yet)              #AFR, no PS
test <- SampleRRs(1,'GBR',noest=noests.yet)              #notifications
test <- SampleRRs(1,'ABW',noest=noests.yet)              #std adj
test <- SampleRRs(1,'BOL',noest=noests.yet)              #AMR problem before as was looking for PS
...
correctRR <- function(cn,MF,RR,verbose=FALSE){
  ...
  Demo[1,] <- unlist(pop[iso3==cn & year==estyr,paste0('e_pop_m',agz),with=FALSE])
  Demo[2,] <- unlist(pop[iso3==cn & year==estyr,paste0('e_pop_f',agz),with=FALSE])
...
test <- SampleRRs(10,'PAK',noest=noests.yet)              #EMR, PS
...
getProps <- function(n=1,cn,verbose=TRUE,shape=FALSE,noest=FALSE){
  smps <- SampleRRs(n=n,iso3=cn,verbose=verbose,noest=noest)     #observed RRs
  if('P' %in% names(smps)){
    ans <- smps$P
    if(shape){
    ...
    Demo[1,] <- unlist(pop[iso3==cn & year==estyr,
                           paste0('e_pop_m',agz),with=FALSE])
    Demo[2,] <- unlist(pop[iso3==cn & year==estyr,
                           paste0('e_pop_f',agz),with=FALSE])
...
## test
getProps(n=50,'PAK',verbose=TRUE,shape=TRUE,noest=noests.yet)
getProps(n=5,'GBR',verbose=TRUE,shape=TRUE,noest=noests.yet)
getProps(n=5,'ABW',verbose=TRUE,shape=TRUE,noest=noests.yet)


## if(noests.yet) knitr::knit_exit()
...
for(cn in unique(est$iso3)){
  if(nrow(est[iso3==cn & year==estyr,])>0){ #skip things without estimates
    ...
    Pdt <- Pdt[,.(prop=mean(value),prop.se=sd(value)),
               by=.(sex,age_group=variable,iso3)]
    prevsplits[[cn]] <- Pdt
  }
}
...
countryplot <- function(cn){
  ...
  ans <- ggplot(data=tmp,aes(x=age_group,y=prop,fill=sex)) +
    coord_flip() +

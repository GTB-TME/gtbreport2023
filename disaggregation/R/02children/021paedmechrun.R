...
testob <- testof[,.(value = sum(value)),
                 by=.(country, replicate, iso3, ARI,LAT)]
...
DO <- fread(here('disaggregation/R/02children/data/paedout_2021.csv'))
...
## p2
ggsave(p2,file=here('disaggregation/R/02children/plots/CountryCompare.pdf'))

p3 <- p2 + geom_text_repel(aes(x=M14,y=median,label=iso3),
                           hjust=0,vjust=0) +
    scale_x_sqrt(label=comma) + scale_y_sqrt(label=comma) +
    coord_fixed(ratio=1,xlim=c(0,250e3),ylim=c(0,250e3))
ggsave(p3,file=here('disaggregation/R/02children/plots/CountryCompareLabeled.pdf'))

p3 <- p + scale_x_log10()+ scale_y_log10()
## p3
ggsave(p3,file=here('disaggregation/R/02children/plots/CountryCompareLog.pdf'))
...
## p4
ggsave(p4,
       file=here('disaggregation/R/02children/plots/CountryCompareSqrt.pdf'),
       height=10,width=10)

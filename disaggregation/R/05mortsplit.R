...
load(file=here('disaggregation/R/02children/data/U.Rdata')) #TODO needs pops also
load(here('disaggregation/R/02children/data/unaids.Rdata'))
...
xlfn <- here('data/mortality/info_TB_may2023.xls')
...
if(!file.exists(mfn)){
  ...
  ## "Netherlands Antilles"         "Netherlands (Kingdom of the)"
  ## Error in `[.data.table`(VR, name == vrbad[i], `:=`(name, newnm)) : 
  ##            Supplied 2 items to be assigned to 64 items of column 'name'. If you wish to 'recycle' the RHS please use rep() to make this intent clear to readers of your code.
  ## others for renaming
  vrbad
...
#' ## HIV by age
#'
#' There is a gender split in Philippe's data, which it is natural to preserve.
...
#'
#' This year we use the age-split UNAIDS estimates
#' 
## ------ HIV data
load(file=here('disaggregation/R/02children/data/U.Rdata'))
U <- U[year==estyr]
SR <- U[,.(sr=sum(hiv_M)/sum(hiv_F)),by=iso3]
U <- merge(U,AA5[,.(age=a5,atb)],by='age',all.x=TRUE)
U <- U[,.(hiv=sum(hiv_A),tot=sum(tot_A)),by=.(iso3,atb)]
U[,variable:=gsub('-','_',atb)]
U[,hivp:=hiv/tot]
HA <- U[!variable %in% c('0_4','5_14')]
HA[,tothiv:=sum(hiv),by=iso3]
HA[,value:=hiv/tothiv]
setkey(HA,iso3)
...
getHIVsplit <- function(cn){
  ...
  if(cn %in% SR$iso3) sr <- SR[iso3==cn]$sr
  sr <- sr/(1+sr)
...
load(file=here('disaggregation/output/mortsplits/HNsplit.Rdata'))
...
## mortregHN[,mort:=mort/sum(mort)] # NOTE this arbitrarily divided by 6
mortregHN[age=='65plus',age:=rev(agz4)[1]]

#' checks
#' 
HNsplit[,sum(mort),by=iso3]
mortregHN[,sum(mort),by=g.whoregion]
